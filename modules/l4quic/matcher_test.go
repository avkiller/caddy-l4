...
// Test_MatchQUIC_DeadlineReturnsNotMatched asserts that when MatchQUIC's
// internal Accept() times out (qContext deadline expires without the
// handshake completing), Match() returns (false, nil) — marking the route
// NotMatched and letting layer4 fall through to the next route — rather
// than (false, ErrConsumedAllPrefetchedBytes), which would block the route
// loop in cx.prefetch() against the outer matchingTimeout.
//
// We exercise this by feeding a truncated QUIC initial packet: it passes
// the byte-level checks at the top of Match(), but never produces a valid
// handshake, so Accept() blocks until QUICAcceptTimeout (100ms) fires.
// With isTesting=true the WaitForMore goroutine short-circuits, so the
// only qContext exit path is DeadlineExceeded.
func Test_MatchQUIC_DeadlineReturnsNotMatched(t *testing.T) {
	isTesting = true
	m := &MatchQUIC{}
	...
	cx := layer4.WrapConnection(out, []byte{}, zap.NewNop())
	go func() {
		// Truncated packet: keeps the magic/long-header bits intact so
		// the byte-level checks pass, but never enough wire data for the
		// QUIC handshake to complete. Accept() will block until the
		// QUICAcceptTimeout deadline fires.
		_, err := in.Write(packet1[:len(packet1)-1])
		assertNoError(t, err)
		_ = in.Close()
	}()
	...
	if matched {
		t.Fatalf("Match returned matched=true for truncated initial; want false")
	}
	if err != nil {
		t.Fatalf("Match returned err=%v for deadline-exceeded path; want nil so layer4 marks NotMatched and falls through (returning ErrConsumedAllPrefetchedBytes here would block the route loop until matchingTimeout)", err)
	}
}
